...
install() {
    ...
    curl -L "$YGG_URL" -o /tmp/yggdrasil.tar.gz

    tar -xzf /tmp/yggdrasil.tar.gz -C "$YGGDRASIL_DIR/bin" --strip-components=1
...
# Запуск Yggdrasil, если он ещё не работает
start_yggdrasil() {
    if [ -f "$PID_FILE" ]; then
        PID=$(cat "$PID_FILE")
...
# Получение собственного IPv6-адреса
get_my_address() {
...
# Проверка, запущен ли скрипт из пайпа (первый запуск)
