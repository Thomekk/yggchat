...
setup() {
    ...
    # Отключаем админский сокет (в Termux нет /var/run)
    sed -i 's|^AdminListen:.*|AdminListen: none|' "$YGG_CONF"
    echo "   ✓ Конфиг создан"

    # Алиасы (исправлен флаг -useconf)
    echo "[5/5] Команды..."
    sed -i '/alias ygg/d' ~/.bashrc 2>/dev/null
    echo "alias yggchat='bash $SCRIPT_FILE'" >> ~/.bashrc
    echo "alias yggstart='yggdrasil -useconf $YGG_CONF &>/dev/null & sleep 5; ip -6 addr show scope global | grep -o \"inet6 [23][0-9a-f:]*\" | cut -d\" \" -f2 | head -1 | tee $ADDR_FILE'" >> ~/.bashrc
...
get_ip() { 
    ip -6 addr show scope global 2>/dev/null | grep -o "inet6 [23][0-9a-f:]*" | head -1 | cut -d" " -f2
}
...
chat() {
    ...
    if [ -z "$IP" ]; then
        ...
        echo ""
        echo "Или вручную:"
        echo "  yggdrasil -useconf $YGG_CONF &"
        echo "  sleep 10"
        echo ""
        exit 1
    fi
    ...
    if [ "$m" = "1" ]; then
        ...
        echo ""
        nc -l -p $PORT -k 2>/dev/null | while read l; do 
            echo -e "\n[Друг]: $l"
            echo -n "> "
        done &
        trap "kill $! 2>/dev/null; echo; exit" INT
        while read -p "> " m; do 
            [ -n "$m" ] && echo "$m" | nc localhost $PORT 2>/dev/null
        done
        ...
        trap "kill $! 2>/dev/null; echo; exit" INT
        while read -p "> " m; do 
            [ -n "$m" ] && echo "$m" | nc $T $PORT 2>/dev/null
        done
    fi
}
